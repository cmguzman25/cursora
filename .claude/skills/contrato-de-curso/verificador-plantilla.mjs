...
/**
 * PLANTILLA. Cópiala a content/courses/<slug>/verificar-lecciones.mjs y ajusta
 * todo lo marcado con AJUSTA. Lo demás funciona tal cual.
 *
 * Verifica lo que se puede medir sin criterio, según `CONTRATO-DE-CLASES.md`.
 * Lo que exige leer (que la clase enseñe una sola idea, que el código funcione,
 * que el fallo listado ocurra de verdad) se queda en la checklist manual.
 *
 * Tres decisiones de esta plantilla que parecen detalles y no lo son:
 *
 ...
 *
 *   node content/courses/<slug>/verificar-lecciones.mjs
 */

// AJUSTA: la carpeta de clases del curso.
const dir = "content/courses/<slug>/lecciones";

// AJUSTA: las secciones obligatorias de cada tipo de clase, en orden.
// Los nombres tienen que coincidir carácter a carácter con los del contrato.
const SECCIONES = {
  A: [
...
// AJUSTA: unidades de lectura por minuto, según la edad del público.
// Calíbralo con la clase piloto antes de escribir el resto del curso.
const UNIDADES_POR_MINUTO = { min: 150, max: 190 };
// AJUSTA si el curso tiene otro contenido lento (vocabulario, fórmulas).
// Una línea de código cuesta leerla como unas diez palabras de prosa.
const PESO_LINEA_CODIGO = 10;
// AJUSTA: los minutos que una cabecera puede declarar.
const MINUTOS_VALIDOS = [8, 9, 10];

// AJUSTA: topes de código por bloque y por clase. Si el curso no lleva
// código, borra estos dos y el apartado 5 entero.
...
// AJUSTA: las palabras que el contrato prohíbe por hacer sentir torpe al alumno.
...
// AJUSTA: según la tabla de edad del skill. 14/12 para niños, 17/15 para
// adolescentes, 20/18 para adultos.
...
for (const file of archivos) {
  ...
  if (repaso === "") {
    ...
    // AJUSTA: cuántas preguntas pide el contrato en el repaso.
  ...
  // AJUSTA si el curso tiene más archivos internos que no debe nombrar.
