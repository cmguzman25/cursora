...
- **Una lección de "analiza preguntas de examen" al cierre de cada módulo 1-4**
  (justo después de la de tablas comparativas): una cantidad de preguntas
  proporcional al peso del dominio (20 / 25 / 35 / 15 para los módulos 1 a 4),
  con el formato real del examen (opción múltiple / respuesta múltiple), explicando por qué
...
- [x] 2.1 Modelo de responsabilidad compartida y cómo cambia según el servicio
      (EC2, RDS, Lambda) (`lecciones/09-responsabilidad-compartida.md`)
- [x] 2.2 Seguridad, gobierno y cumplimiento: AWS Artifact, cifrado en tránsito/reposo,
      CloudWatch, CloudTrail, AWS Config (`lecciones/10-gobierno-y-cumplimiento.md`)
- [x] 2.3 Gestión de accesos: IAM (usuarios, grupos, roles, políticas), usuario root,
      MFA, IAM Identity Center, principio de menor privilegio (`lecciones/11-iam-y-gestion-de-accesos.md`)
- [x] 2.4a Componentes de seguridad — protección contra ataques: AWS Shield,
      AWS WAF, Firewall Manager, AWS Marketplace y los recursos de seguridad de
      AWS (`lecciones/12-proteccion-contra-ataques.md`)
- [x] 2.4b Componentes de seguridad — detección y auditoría: GuardDuty,
      Inspector, Macie, Security Hub y Trusted Advisor
      (`lecciones/13-deteccion-y-auditoria.md`)
- [x] ★ 2.5 Tablas comparativas: GuardDuty vs. Inspector vs. Macie vs. Security
      Hub vs. Trusted Advisor — qué hace cada uno — y encriptación en tránsito
      vs. en reposo (`lecciones/14-comparativas-seguridad.md`)
- [x] 2.6 Analiza preguntas de examen: 25 preguntas de seguridad y cumplimiento,
      con el porqué de cada opción correcta e incorrecta
      (`preguntas/modulo-2.ts` — lección interactiva, no Markdown)
